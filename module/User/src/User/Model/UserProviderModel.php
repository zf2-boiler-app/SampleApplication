...
class UserProviderModel extends \Database\Db\TableGateway\TableGateway{
	 ...
	 */
	public function __construct(\Zend\Db\Adapter\Adapter $oAdapter,\User\Model\UserModel $oUserModel){
		$this->userModel = $oUserModel;
		parent::__construct(
			'users_providers',
			$oAdapter,
			null,
			new \Zend\Db\ResultSet\ResultSet(\Zend\Db\ResultSet\ResultSet::TYPE_ARRAYOBJECT,new \User\Entity\UserProviderEntity())
		);
	}
	 ...
	 * @throws \Exception
	 * @return \User\Entity\UserEntity|NULL
	 */
	public function getUser($sProviderId, $sProvider){
		...
		if(!($oUserProvider = $this->select(array(
			...
			'provider' => $sProvider
		))->current()) instanceof \User\Entity\UserProviderEntity)return null;
		elseif(($oUser = $this->userModel->select(array(
			'user_id' => $oUserProvider->getUserId()
		))->current()) instanceof \User\Entity\UserEntity)return $oUser;
	 ...
	 * @throws \Exception
	 * @return \User\Model\UserProviderModel
	 */
